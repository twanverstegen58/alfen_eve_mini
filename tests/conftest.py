...
from custom_components.alfen_eve_mini.const import (
...
def mock_setup_entry_fixture():
    ...
    with patch(
        "custom_components.alfen_eve_mini.async_setup_entry", return_value=True
    ) as mock_setup:
        yield mock_setup
...
def mock_aiohttp_session_fixture():
    ...
    with patch("custom_components.alfen_eve_mini.coordinator.ClientSession") as mock_session_class:
        session = MagicMock()
...
def mock_tcp_connector_fixture():
    ...
    with patch("custom_components.alfen_eve_mini.coordinator.TCPConnector") as mock_connector:
        connector = MagicMock()
...
def mock_ssl_context_fixture():
    """Mock SSL context."""
    with patch("custom_components.alfen_eve_mini.coordinator.get_default_context") as mock_ssl:
        context = MagicMock()
